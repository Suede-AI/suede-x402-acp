/**
 * Tests for the suede_stems v2 handler.
 */
...
const { handle } = await import("./suede_stems.js");
...
describe("suede_stems handler", () => {
  it("registers itself with the dispatch registry", () => {
    expect(getHandler("suede_stems")).toBe(handle);
  });
  ...
  it("calls extractStems with basic tier and returns zip envelope", async () => {
    extractStems.mockResolvedValueOnce({ url: "https://cdn/stems.zip" });
  ...
  it("maps BACKEND_UNAVAILABLE on backend errors", async () => {
    extractStems.mockRejectedValueOnce(new SuedeEndpointUnavailableError("POST /v1/stems"));
    try {
